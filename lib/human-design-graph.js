...
function centreCorners(id) {
  const v = centreVertices(id);
  return v.map((p, i) => {
    const a = v[(i + v.length - 1) % v.length],
      b = v[(i + 1) % v.length];
    const toward = (q) => {
      const f = Math.min(0.22, 15 / Math.hypot(q.x - p.x, q.y - p.y));
      return { x: p.x + (q.x - p.x) * f, y: p.y + (q.y - p.y) * f };
    };
    return { p, entry: toward(a), exit: toward(b) };
  });
}
function centrePath(id) {
  return (
    centreCorners(id)
      .map(
        ({ p, entry, exit }, i) =>
          `${i ? "L" : "M"} ${entry.x} ${entry.y} Q ${p.x} ${p.y} ${exit.x} ${exit.y}`,
      )
      .join(" ") + " Z"
  );
}
function centreOutline(id) {
  return centreCorners(id).flatMap(({ p, entry, exit }) =>
    Array.from({ length: 17 }, (_, i) => {
      const t = i / 16,
        s = 1 - t;
      return {
        x: s * s * entry.x + 2 * s * t * p.x + t * t * exit.x,
        y: s * s * entry.y + 2 * s * t * p.y + t * t * exit.y,
      };
    }),
  );
}
...
function channelPath(a, b) {
  ...
  const special = {
    "20-34": [240, 545, 235, 720],
    "20-57": [210, 525, 185, 675],
    "16-48": [165, 480, 115, 655],
    "12-22": [610, 480, 645, 685],
    "35-36": [655, 490, 685, 700],
    "21-45": [520, 510, 550, 615],
    "26-44": [430, 730, 320, 710],
    "10-34": [250, 645, 265, 720],
    "10-57": [240, 605, 185, 690],
    "34-57": [255, 730, 225, 690],
    "18-58": [95, 970, 225, 1040],
    "28-38": [185, 985, 265, 1005],
    "32-54": [250, 910, 290, 938],
    "19-49": [540, 950, 570, 875],
    "39-55": [610, 985, 620, 930],
    "30-41": [675, 1060, 705, 950],
  };
  let control = special[keyOf(a, b)];
  // Special controls are ordered for the PAIRS orientation, except 30–41.

  const centerPair = [GATE_CENTRE[a], GATE_CENTRE[b]].sort().join("/");
  if (
    [
      "Ajna/Head",
      "Ajna/Throat",
